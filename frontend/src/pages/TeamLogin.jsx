...
import {
  FaUsers, FaLock, FaEye, FaEyeSlash, FaEnvelope,
...
export default function TeamLogin() {
  ...
  const handleForgotOtp = async e => {
    e.preventDefault();
    if (fpOtp.join('').length < 6) { toast.error('Enter the 6-digit OTP'); return; }
        ...
        {step === 'otp' && (
          <motion.div key="otp" className="auth-card" {...slideIn}>
            ...
            <form onSubmit={handleLoginOtp}>
              ...
              <button type="submit" className="btn auth-btn" disabled={loading || loginOtp.join('').length < 6}
                style={{ background: '#1a5c38', color: 'white', width: '100%', justifyContent: 'center' }}>
                {loading ? 'Verifying...' : '✅ Verify & Login'}
              </button>
            </form>
            <button className="auth-back-btn" onClick={() => { setStep('login'); setLoginOtp(['','','','','','']); }}>
              <FaArrowLeft /> Back
            </button>
          </motion.div>
        )}
            ...
            <form onSubmit={handleResetPassword}>
              ...
              <div className="form-group">
                ...
                <input type="password" value={confirmNewPw}
                  onChange={e => setConfirmNewPw(e.target.value)} placeholder="Re-enter new password" required />
              </div>
              <button type="submit" className="btn btn-primary auth-btn" disabled={loading}>
